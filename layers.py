...
class HiddenLayer(object):
    def __init__(self, x, in_dim, out_dim, id, W=None, b=None, activation=T.tanh):
        if W is None:
            ...
            W = theano.shared(value=W_values, name=id+'weights', borrow=True)
        if b is None:
            b_values = np.zeros((out_dim,), dtype=theano.config.floatX)
            b = theano.shared(value=b_values, name=id+'biases', borrow=True)
